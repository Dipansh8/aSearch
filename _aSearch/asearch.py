...
# 1) openFile the txt file (e.g., starting point, ending point, obstacle locations)
# 2) Run a-star search
...
class aStarSearch(object):
    def __init__(self, fname, verbose=False):
        ...
        self.vertices = set() #obstacles coordinates
        self.obstacles = []
    ...
    # member function openFile() openFiles the txt file into member objects
    def openFile(self):
        with open(self.fname, 'r') as f:
            ...
            for obstacle_line in lines[3:]:
                ...
                print("Data stored in obstacle line :")
                print(obstacle_line)
                # obstacle_points = zip(obstacle_line[0::2], obstacle_line[1::2]) #zip ? 
                # #print(obstacle_line[0::2])       # print every 2nd value starting from 0  
                # for x,y in obstacle_points:
                #     #print(x,y)
                #     self.vertices.add(Point(x,y))
                #     self.obstacles[-1].append(Point(x,y)) 
                #last_index = len(obstacle_line) - 1 #can be fixed as always there will be 8 values 
                i=0
                while i<7 :
                    self.vertices.add(Point(obstacle_line[i],obstacle_line[i+1]))
                    self.obstacles[-1].append(Point(obstacle_line[i],obstacle_line[i+1])) 
                    i+=2
        if self.verbose:
            print(self.vertices)
